...
const categoriesGrid = ({
  ...
  const sendRequest = e => {
    ...
    if (route !== pathname) {
      ...
      startCategoriesRotatingSpinner()
      return apiGetProductsByCategory(query).then(results =>
        addCategoriesProducts(route, results.products, results.filters)
      )
    } else {
      return false
    }
  ...
  const categoryItems = categories.map((categorie, index) => (
    <Link
      ...
      className={`categorie-item ${
        pathname === '/categorias/' + categorie.to ? 'categorie-item-active' : null
      }`}
      onClick={sendRequest}
    >
  ...
  return (
    <div className="categories-container">
      <div className="categories-grid">{categoryItems}</div>
    </div>
  )
}
