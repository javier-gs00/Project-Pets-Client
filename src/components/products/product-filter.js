    ...
    return (
      <div key={index} className="filter">
        <span className="filter-name">
          {filter.id !== '' ? capitalizeFirstLetter(filter.id) : 'No especifica'}
        </span>
        <input
          type="checkbox"
          id={filter.id}
        ...
        />
        <label htmlFor={filter.id} className="lbl" />
      </div>
    )
  }
      ...
      <div id="filters" className="filters-container">
        ...
        <div className="filter">
          ...
          <div className="filter-svg-container">
            <div id="null_sort" className="filter-svg-item" onClick={props.handleSort}>
              <FontAwesomeIcon icon="undo-alt" />
            </div>
            <div id="desc_sort" className="filter-svg-item" onClick={props.handleSort}>
              <FontAwesomeIcon icon="sort-numeric-down" />
            </div>
            <div id="asc_sort" className="filter-svg-item" onClick={props.handleSort}>
        ...
        {petFilters}
        {props.path.split('/')[1] === 'categorias' ? null : (
          <span className="filter-title">Categorías</span>
        )}
        {props.path.split('/')[1] === 'categorias' ? null : categoryFilters}
      </div>
    </div>
  )
}
