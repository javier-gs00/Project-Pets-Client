...
const ProductNav = ({ activePage, filteredProductsFoundLength, handleActivePageChange }) => {
  let buttonsArray = []
  if (filteredProductsFoundLength <= 20)
    return (
      <div className="products-nav-container" id="products-nav">
        <div className="products-nav">
          <button key="0" onClick={handleActivePageChange}>
            1
          </button>
        </div>
      </div>
  ...
  const pageQuantity =
    filteredProductsFoundLength % 20 === 0
  ...
  buttonsArray.push(
    pageQuantity > 1 ? (
      <button
        key="2"
        className={
          activePage !== 1 && activePage !== pageQuantity
            ? 'products-nav-active'
  ...
  buttonsArray.push(
    pageQuantity > 2 ? (
      <button
        ...
        onClick={handleActivePageChange}
      >
        {activePage === pageQuantity ? pageQuantity : activePage === 1 ? 3 : activePage + 1}
      </button>
    ) : null
  )
  buttonsArray.push(
    activePage !== pageQuantity ? (
      <button key="4" onClick={handleActivePageChange}>
        {'>'}
      </button>
    ) : null
  )
  ...
  return (
    <div className="products-nav-container" id="products-nav">
      <div className="products-nav">{buttonsArray.map(button => button)}</div>
    </div>
  )
}
