...
class SearchContainer extends Component {
  componentDidMount() {
    ...
    const routeName =
      location.pathname === '/productos' ? location.pathname : '/' + location.pathname.split('/')[1]
    ...
    if (products.length > 0) {
      ...
      return apiProductsSearch(parsedUrlQuery.query).then(results => {
        addProducts(fullRouteName, parsedUrlQuery.query, results.products, results.filters)
      })
    } else {
      ...
      return apiProductsSearch().then(results => {
        addProducts(routeName, 'royal canin maxi', results.products, results.filters)
      })
    }
  }
  ...
  render() {
    const { products, isLoading, activePage, searchValue, filters, history } = this.props
    ...
    return (
      <div id="main" className="main">
        <div className="products-main-container">
          ...
          <Switch>
            <Route
              path="/productos/resultados/:id"
              render={props => <AsyncSingleProductView {...props} />}
            />
            <Route
